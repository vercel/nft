...
it('should correctly print trace from cli', async () => {
  const { stderr, stdout } = await exec(`node ../dist/cli.js print ${inputjs}`, { cwd: __dirname });
  if (stderr) {
...
it('should correctly build dist from cli', async () => {
  const { stderr } = await exec(`node ../dist/cli.js build ${inputjs}`, { cwd: __dirname });
  if (stderr) {
    throw new Error(stderr);
  }
...
it('should correctly print help when unknown action is used', async () => {
  const { stderr, stdout } = await exec(`node ../dist/cli.js unknown ${inputjs}`, { cwd: __dirname });
  if (stderr) {
    throw new Error(stderr);
  }
